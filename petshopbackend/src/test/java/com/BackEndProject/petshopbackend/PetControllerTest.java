...
public class PetControllerTest {
    ...
    void testDeletePet() throws Exception {
        ...
        mockMvc.perform(delete("/pet/1"))
                .andExpect(status().isOk())
                .andExpect(content().string("Pet not found this id: 1"));
    }
    @Test
    void testUpdatePet() throws Exception{
            when(petService.updatePet(Mockito.anyLong(),any(Pet.class)))
                    .thenReturn(pet);

            mockMvc.perform(put("/pet/1")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"pet\": \"Buddy\"}"))
                    .andExpect(status().isOk()).andExpect(content().string("Succesfully changed"));
